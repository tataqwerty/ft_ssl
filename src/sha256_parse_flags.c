...
static char	s_handler(char **cur, char *next, int *i, t_sha_256 *flags)
{
	(*cur)++;
	if (**cur)
	{
		hash_handler(ft_strdup(*cur), flags, sha_256_hash, sha_256_output);
		*cur += ft_strlen(*cur);
	}
	else if (next)
	{
		(*i)++;
		hash_handler(ft_strdup(next), flags, sha_256_hash, sha_256_output);
	}
	else
	{
...
static char	p_handler(t_sha_256 *flags)
{
	char	*input_value;
	...
	hash_handler(input_value, flags, sha_256_hash, sha_256_output);
	return (1);
}
...
static char	dispatcher_flags(char **cur, char *next, int *i, t_sha_256 *flags)
{
	if (**cur == 'q')
		flags->q = 1;
	else if (**cur == 'r')
...
static char	parse_flags_arg(char *cur, char *next, int *i, t_sha_256 *flags)
{
	char	flag_output;
